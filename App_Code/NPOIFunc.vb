...
Public Class NPOIFunc

    Public Sub CopyCell(ByRef oldCell As HSSFCell, ByRef newCell As HSSFCell, Optional ByVal styleOnly As Boolean = False)
        'If the old cell is null, exit
        If oldCell Is Nothing Then
            Exit Sub
        End If

        Dim workbook As HSSFWorkbook = newCell.Sheet.Workbook

        'Copy style from old cell and apply to new cell
        'Dim newCellStyle As HSSFCellStyle = workbook.CreateCellStyle()
    ...
    Public Sub CopyRange(ByRef sourceSheet As HSSFSheet, ByRef destSheet As HSSFSheet, ByRef oldRange As CellRangeAddress, ByVal destRow As Integer, ByVal destCol As Integer, Optional ByVal styleOnly As Boolean = False)
        ...
        For i As Integer = oldRange.FirstRow To oldRange.LastRow
            For j As Integer = oldRange.FirstColumn To oldRange.LastColumn
                Dim sourceRow As HSSFRow = sourceSheet.GetRow(i)
                If sourceRow IsNot Nothing Then
                    Dim oldCell As HSSFCell = sourceSheet.GetRow(i).GetCell(j)
                    If oldCell IsNot Nothing Then
                        Dim newRow As HSSFRow = destSheet.GetRow(i - rowDiff)
                        If newRow Is Nothing Then
                            newRow = destSheet.CreateRow(i - rowDiff)
                        End If
                        Dim newCell As HSSFCell = newRow.GetCell(j - colDiff)
    ...
    Public Sub CopyRow(ByRef workbook As HSSFWorkbook, ByRef sourceSheet As HSSFSheet, ByRef destSheet As HSSFSheet, ByVal sourceRowNum As Integer, ByVal destRowNum As Integer, Optional ByVal styleOnly As Boolean = False)
        'Get the source / new row
        Dim newRow As HSSFRow = destSheet.GetRow(destRowNum)
        Dim sourceRow As HSSFRow = sourceSheet.GetRow(sourceRowNum)
        ...
        For i As Integer = 0 To sourceRow.LastCellNum - 1
            ...
            Dim oldCell As HSSFCell = sourceRow.GetCell(i)
            If oldCell IsNot Nothing Then
                Dim newCell As HSSFCell = newRow.CreateCell(i)
    ...
    Public Function getCell(ByRef ExcelWSObj As HSSFSheet, ByVal rowNo As Integer, ByVal colNo As Integer) As HSSFCell
    ...
    Public Sub setCellValue(ByRef ExcelWSObj As HSSFSheet, ByVal rowNo As Integer, ByVal colNo As Integer, ByRef cellValue As String, Optional ByVal itemType As String = "")
        Dim ExcelRow As HSSFRow
        Dim ExcelCell As HSSFCell
    ...
    Public Sub InsertRows(ByRef sheet1 As HSSFSheet, fromRowIndex As Integer, rowCount As Integer)
        ...
        For rowIndex As Integer = fromRowIndex To fromRowIndex + (rowCount - 1)
            Dim rowSource As HSSFRow = sheet1.GetRow(rowIndex + rowCount)
            Dim rowInsert As HSSFRow = sheet1.CreateRow(rowIndex)
            'rowInsert.Height = rowSource.Height
            For colIndex As Integer = 0 To rowSource.LastCellNum - 1
                Dim cellSource As HSSFCell = rowSource.GetCell(colIndex)
                Dim cellInsert As HSSFCell = rowInsert.CreateCell(colIndex)
    ...
    Public Sub removeRow(sheet As HSSFSheet, rowIndex As Integer)
        ...
        If rowIndex = lastRowNum Then
            Dim removingRow As HSSFRow = sheet.GetRow(rowIndex)
