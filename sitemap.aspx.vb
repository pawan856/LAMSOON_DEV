Imports System.Web
Imports System.Net
Imports System.IO
Imports System.Data

Partial Class sitemap
    Inherits System.Web.UI.Page

    Private gDB As New GlobalDBFunc
    Private ar As New AccessRightUtils

    Private menu_code As String
    Private moduelCollection As New ArrayList
    Private prgmIdCollection As New ArrayList
    Private moduelTypeCollection As New ArrayList
    Private noOfSubCollection As New ArrayList

    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        menu_code = Request("menu_code")

        If Session("usr_id") Is Nothing Or Session("usr_id") = "" Then
            ar.Force_PageEndCtrlClear(Me, False)
            Me.Visible = False
            Exit Sub
        End If

        Dim SQLString As String = ""
        Dim dt As New DataTable

        SQLString = "SELECT fun_type, fun_code, fun_eng_name, fun_chi_name, fun_display_seq, fun_web_page,(select count(*) from wms_function a where a.fun_type = 'SCREEN' and a.fun_parent_code = m.fun_code) as noOfSub from wms_function m "
        If Session("usr_type") = "S" Then
            SQLString = SQLString & "where fun_type = 'MENU' "
        ElseIf Session("usr_type") = "C" Then
            SQLString = SQLString & "where fun_type = 'MENU_CUST' "
        ElseIf Session("usr_type") = "T" Then
            SQLString = SQLString & "where fun_type = 'MENU_STOERER' "
        ElseIf Session("usr_type") = "V" Then
            SQLString = SQLString & "where fun_type = 'MENU_VENDOR' "
        Else
            SQLString = SQLString & "where 1 = 0 "
        End If

        SQLString = SQLString & " OR (fun_type = 'SCREEN' and exists (select 1 from wms_function T where T.fun_code = M.fun_parent_code "
        If Session("usr_type") = "S" Then
            SQLString = SQLString & "and T.fun_type = 'MENU' "
        ElseIf Session("usr_type") = "C" Then
            SQLString = SQLString & "and T.fun_type = 'MENU_CUST' "
        ElseIf Session("usr_type") = "T" Then
            SQLString = SQLString & "and T.fun_type = 'MENU_STOERER' "
        ElseIf Session("usr_type") = "V" Then
            SQLString = SQLString & "and T.fun_type = 'MENU_VENDOR' "
        Else
            SQLString = SQLString & "and 1 = 0 "
        End If

        SQLString = SQLString & ") and exists (select 1 from wms_user_group_alloc uga, wms_sec_access sa " & _
        "where sa.sec_access = 'Y' and sa.grp_code = uga.grp_code and sa.fun_code = m.fun_code and uga.usr_id = '" & Session("usr_id") & "')) "
        SQLString = SQLString & " order by fun_display_seq"

        dt = gDB.getDataTable(SQLString)
        For i As Integer = 0 To dt.Rows.Count - 1
            If Session("gLang") = "E" Then
                moduelCollection.Add(dt.Rows(i).Item("fun_eng_name").ToString)
            Else
                moduelCollection.Add(dt.Rows(i).Item("fun_chi_name").ToString)
            End If
            prgmIdCollection.Add(dt.Rows(i).Item("fun_web_page").ToString & dt.Rows(i).Item("fun_code").ToString)
            moduelTypeCollection.Add(dt.Rows(i).Item("fun_type").ToString)
            noOfSubCollection.Add(dt.Rows(i).Item("noOfSub").ToString)
        Next

        Dim iL As Integer = 1

        Dim htmlString As String = "<table border=""0"" cellspacing=""0"" cellpadding=""0""  width=""100%"" >" & _
                                    "<tr><td colspan=""2"">&nbsp;</td></tr><tr><td width=""10%"" class=""main_page_text"">&nbsp;" & _
                                    "</td><td width=""74%"" class=""main_page_text""><table width=""100%"" border=""0"" cellspacing=""0"" " & _
                                    "cellpadding=""5""></table></td></tr>"

        Dim htmlEndTag As String = ""
        Dim prgmLink As String = ""
        Dim skipItem As String = ""
        If moduelCollection.Count > 0 Then
            htmlString = htmlString & "<tr><td width=""10%"" class=""main_page_text"">&nbsp;</td><td width=""74%"" class=""main_page_text"">" & _
                        "<table cellspacing=1 cellpadding=5 border=0 bgcolor=""#CCCCCC"" align=""center"" width=100%>"

            For x As Integer = 0 To moduelCollection.Count - 1
                prgmLink = ""
                htmlEndTag = ""

                If Session("usr_type") = "S" Then
                    If noOfSubCollection.Item(x) <= 1 And moduelTypeCollection.Item(x).ToString = "MENU" Then
                        skipItem = "Y"
                    End If
                ElseIf Session("usr_type") = "C" Then
                    If noOfSubCollection.Item(x) <= 1 And moduelTypeCollection.Item(x).ToString = "MENU_CUST" Then
                        skipItem = "Y"
                    End If
                End If

                If skipItem <> "Y" Then
                    If moduelTypeCollection.Item(x).ToString = "SCREEN" Then
                        If skipItem = "N" Then
                            htmlString = htmlString & "<tr bgcolor=""#F0F0F0"">"
                            htmlString = htmlString & "<td onMouseOver=""this.bgColor='#B8B8B8'"" onMouseOut=""this.bgColor='#F0F0F0'"" class=""main_menu"">"
                            htmlString = htmlString & "<a href=""" & prgmLink & prgmIdCollection.Item(x).ToString & """ style="""" class=""main_menu"">"
                            htmlString = htmlString & moduelCollection.Item(x).ToString & "</a>"
                        Else
                            htmlString = htmlString & "<tr bgcolor=""#E3E3E3"">"
                            htmlString = htmlString & "<td onMouseOver=""this.bgColor='#B8B8B8'"" onMouseOut=""this.bgColor='#E3E3E3'"" class=""main_menu"">"
                            htmlString = htmlString & "&nbsp;&nbsp;&nbsp;-"
                            htmlString = htmlString & "<a href=""" & prgmLink & prgmIdCollection.Item(x).ToString & """ style="""" class=""main_menu"">"
                            htmlString = htmlString & moduelCollection.Item(x).ToString & "</a>"
                        End If

                    Else
                        htmlString = htmlString & "<tr bgcolor=""#F0F0F0"">"
                        htmlString = htmlString & "<td onMouseOver=""this.bgColor='#B8B8B8'"" onMouseOut=""this.bgColor='#F0F0F0'"" class=""main_menu"">"
                        htmlString = htmlString & moduelCollection.Item(x).ToString
                    End If
                        htmlString = htmlString & "</td></tr>"

                        htmlString = htmlString & htmlEndTag
                        skipItem = ""
                    Else
                        skipItem = "N"
                    End If
            Next
            htmlString = htmlString & "</table></td></tr>"

        End If

        htmlString = htmlString & "</table>"

        menuform.InnerHtml = htmlString

    End Sub
End Class
