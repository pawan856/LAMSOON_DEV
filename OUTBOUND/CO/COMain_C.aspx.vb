...
Partial Class OUTBOUND_COMain_C
    ...
    Private gvCol() As String = {"COD_DISP_SEQ", "BTNSPLIT", "COD_PLANT", "COD_REF_NO", "COD_CARTON_NO", "COD_PALLET_NO", "COD_BATCH_NO", "COD_LOT_NO",
                                 "COD_VND_CODE", "COD_ITM_CODE", "ITM_SKU_NO", "COD_ITM_DESC", "ITM_DESC", "COD_EXPIRY_DATE", "COD_MANU_DATE", "COD_PACKING", "COD_PACK_KEY", "COD_QTY", "COD_OS_QTY",
                                 "COD_PCS_CARTON", "COD_CARTON_PALLET", "COD_UOM", "COD_PCS_UOM", "COD_TOTPCS", "COD_TOT_WGT", "COD_TOT_CBM", "COD_REM"}
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        ar = New AccessRightUtils("OB_CO_C", Session("usr_id"), Me)
        ...
        ElseIf Session("gLang") = "C" Then
            ...
            lbl_CO_CODE.Text = "訂單號碼:"
            lbl_CO_STATUS.Text = "狀態:"
            'lbl_STORER_CODE.Text = "貨主:"
            lbl_CO_CUS_REF_NO.Text = "參考編號:"
        ...
        ar.addColDef("COD_REF_NO", "cod_ref_no", colIdx_StartWith)
        ...
        ar.setFieldCustomize(Me, "OB_CO_C", STORER_CODE.Value, "WMS_CUST_ORDER")

        If GridView1.Rows.Count > 0 Then
            ar.setGVCustomize(GridView1, "OB_CO_C", STORER_CODE.Value, "WMS_CUST_ORDER_D", gvCol)
        End If
        ...
        Select Case e.CommandName
            Case "SplitItem"
                If cU.gfBuildDataTableforGridView(dt, GridView1, True) Then
                    ...
                    Dim seq_string As String = "select MAX(TO_NUMBER(COD_SEQ)) + 1 from WMS_CUST_ORDER_D " & _
                    ...
                    dt.Rows(rowNum + 1).Item("COD_REF_NO") = dt.Rows(rowNum).Item("COD_REF_NO")
                    ...
                    dt.Rows(rowNum + 1).Item("ori_batch") = dt.Rows(rowNum).Item("ori_batch")
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView1.RowDataBound
        Select Case e.Row.RowType
            Case DataControlRowType.DataRow
                ...
                CType(e.Row.FindControl("cod_ref_no"), TextBox).Text = DataBinder.Eval(e.Row.DataItem, "cod_ref_no").ToString.Trim
                ...
                uiFun.load_dropdown(CType(e.Row.FindControl("cod_vnd_code"), DropDownList), _
                                    "select distinct vnd_code " & _
                                    "from wms_alt_vend_item " & _
                                    "where " & _
                                    "wms_alt_vend_item.imp_code = '" & gU.dbEncode(Session("IMP_CODE")) & "' " & _
                                    "and wms_alt_vend_item.storer_code = '" & gU.dbEncode(STORER_CODE.Value) & "' " & _
                                    "and wms_alt_vend_item.itm_code = '" & gU.dbEncode(DataBinder.Eval(e.Row.DataItem, "cod_itm_code").ToString.Trim) & "' " & _
                                    "and wms_alt_vend_item.pack_key = '" & gU.dbEncode(DataBinder.Eval(e.Row.DataItem, "cod_pack_key").ToString.Trim) & "' " & _
                ...
                CType(e.Row.FindControl("cod_qty"), TextBox).Text = cU.FormatIntegerString(DataBinder.Eval(e.Row.DataItem, "cod_qty").ToString.Trim)
                CType(e.Row.FindControl("cod_os_qty"), Label).Text = gU.decodeEmptyCInt(DataBinder.Eval(e.Row.DataItem, "cod_qty").ToString.Trim, 0) - gU.decodeEmptyCInt(DataBinder.Eval(e.Row.DataItem, "cod_post_qty").ToString.Trim, 0)
                ...
                '                                "STORER_CODE = '" & STORER_CODE.Value.Trim & "' AND " & _
                ...
                'uiFun.load_dropdown(CType(e.Row.FindControl("cod_batch_no"), DropDownList), "select dc_date_code from wms_date_code where imp_code = '" & gU.dbEncode(Session("imp_code")) & "' and storer_code = '" & gU.dbEncode(STORER_CODE.Value) & "' order by 1", "dc_date_code", "dc_date_code", , Session("gSelectLabel"))
                ...
                Dim cod_batch_no As String = DataBinder.Eval(e.Row.DataItem, "cod_batch_no").ToString.Trim

                If cod_batch_no.Length > 6 Then
                    If cod_batch_no.Substring(0, 6).Equals("@B#_E_") OrElse cod_batch_no.Substring(0, 6).Equals("@B#_M_") Then
                        CType(e.Row.FindControl("cod_batch_no"), Label).Text = ""
                    Else
                ...
                CType(e.Row.FindControl("COD_MANU_DATE"), HiddenField).Value = DataBinder.Eval(e.Row.DataItem, "COD_MANU_DATE").ToString.Trim
                CType(e.Row.FindControl("dsp_COD_MANU_DATE"), Label).Text = DataBinder.Eval(e.Row.DataItem, "COD_MANU_DATE").ToString.Trim
                CType(e.Row.FindControl("COD_EXPIRY_DATE"), HiddenField).Value = DataBinder.Eval(e.Row.DataItem, "COD_EXPIRY_DATE").ToString.Trim
                CType(e.Row.FindControl("dsp_COD_EXPIRY_DATE"), Label).Text = DataBinder.Eval(e.Row.DataItem, "COD_EXPIRY_DATE").ToString.Trim

                CType(e.Row.FindControl("COD_LOT_NO"), TextBox).Text = DataBinder.Eval(e.Row.DataItem, "COD_LOT_NO").ToString.Trim
        ...
        If validateAll() Then
            ...
            Try
                If Session("pagemode") = "N" Then
                    ...
                    dupSQL = "select 1 from wms_cust_order " & _
                            "where co_code = '" & gU.dbEncode(nextNo) & "' " & _
                    ...
                    If dupTbl.Rows.Count = 0 Then
                        sql_string = "insert into wms_cust_order ( " & _
                                        "co_code, imp_code, storer_code, " & _
                                        "co_status, co_cus_ref_no, co_project_no, " & _
                                        "co_date, cus_code, cus_name, " & _
                                        "co_addr1, co_addr2, co_addr3," & _
                                        "co_area_del, co_region_del, co_country_del, " & _
                                        "co_cus_cont, co_cus_cont_tel, co_rem, co_track_no, co_ftrack_no, " & _
                                        "co_inv_no, co_cust_inv_no,CO_SHIP_MODE,CO_TARGET_DELDATE," & _
                                        ...
                                        gU.convdbNVCData(gU.dbEncode(CO_STATUS.Text)) & "," & gU.convdbNVCData(gU.dbEncode(CO_CUS_REF_NO.Text)) & "," & gU.convdbNVCData(gU.dbEncode(CO_PROJECT_NO.Text)) & ", " & _
                                        gU.convdbDate(gU.dbEncode(CO_DATE.Text)) & "," & gU.convdbNVCData(gU.dbEncode(CUS_CODE.Text)) & "," & gU.convdbNVCData(gU.dbEncode(CUS_NAME.Text)) & ", " & _
                                        gU.convdbNVCData(gU.dbEncode(CO_ADDR1.Text)) & "," & gU.convdbNVCData(gU.dbEncode(CO_ADDR2.Text)) & "," & gU.convdbNVCData(gU.dbEncode(CO_ADDR3.Text)) & ", " & _
                                        gU.convdbNVCData(gU.dbEncode(CO_AREA_DEL.Text)) & "," & gU.convdbNVCData(gU.dbEncode(CO_REGION_DEL.Text)) & "," & gU.convdbNVCData(gU.dbEncode(CO_COUNTRY_DEL.Text)) & ", " & _
                                        gU.convdbNVCData(gU.dbEncode(CO_CUS_CONT.Text)) & "," & gU.convdbNVCData(gU.dbEncode(CO_CUS_CONT_TEL.Text)) & "," & gU.convdbNVCData(gU.dbEncode(CO_REM.Text)) & ", " & _
                                        gU.convdbNVCData(gU.dbEncode(CO_TRACK_NO.Text)) & "," & gU.convdbNVCData(gU.dbEncode(CO_FTRACK_NO.Text)) & ", " & _
                                        gU.convdbNVCData(gU.dbEncode(CO_INV_NO.Text)) & "," & gU.convdbNVCData(gU.dbEncode(CO_CUST_INV_NO.Text)) & "," & gU.convdbNVCData(gU.dbEncode(CO_SHIP_MODE.SelectedValue)) & "," & _
                                        gU.convdbDate(gU.dbEncode(CO_TARGET_DELDATE.Text)) & "," & _
                        ...
                        If cU.gfBuildDataTableforGridView(dt, GridView1, True) Then
                            ...
                            For Each rows As DataRow In dt.Rows
                                ...
                                Select Case rows.Item("mFlag")
                                    Case "N"
                                        itemSQL = "insert into wms_cust_order_d ( " & _
                                                    "co_code, imp_code, storer_code, cod_seq, cod_disp_seq, " & _
                                                    "cod_job_no, cod_ref_no, cod_cust_code, " & _
                                                    "cod_itm_parent, cod_itm_code, cod_pack_key, cod_itm_desc, " & _
                                                    "cod_packing, cod_qty, cod_uom, cod_pcs_uom, " & _
                                                    "cod_totpcs, cod_tot_wgt, cod_tot_cbm, cod_rem, " & _
                                                    "cod_pallet_no, cod_vnd_code, cod_ref_seq, cod_batch_no, " & _
                                                    "cod_lot_no, cod_carton_no, cod_plant, cod_pcs_carton, cod_carton_pallet, " & _
                                                    "COD_EXPIRY_DATE, COD_MANU_DATE," & _
                                                    "sys_cb, sys_cd, sys_lub, sys_lud) " & _
                                                    "values ( " & _
                                                    gU.convdbNVCData(gU.dbEncode(nextNo)) & ",'" & imp_code & "'," & gU.convdbNVCData(gU.dbEncode(STORER_CODE.Value)) & ", " & _
                                                    gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_seq").ToString.Trim, ""))) & ", " & _
                                                    gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_disp_seq").ToString.Trim, ""))) & ", " & _
                                                    gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_job_no").ToString.Trim, ""))) & ", " & _
                                                    gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_ref_no").ToString.Trim, ""))) & ", " & _
                                                    gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_cust_code").ToString.Trim, ""))) & ", " & _
                                                    gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_itm_parent").ToString.Trim, ""))) & ", " & _
                                                    gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_itm_code").ToString.Trim, ""))) & ", " & _
                                                    gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_pack_key").ToString.Trim, ""))) & ", " & _
                                                    gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_itm_desc").ToString.Trim, ""))) & ", " & _
                                                    gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_packing").ToString.Trim, ""))) & ", " & _
                                                    gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("cod_qty").ToString.Trim, "0")) & ", " & _
                                                    gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_uom").ToString.Trim, ""))) & ", " & _
                                                    gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("cod_pcs_uom").ToString.Trim, "0")) & ", " & _
                                                    gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("cod_totpcs").ToString.Trim, "0")) & ", " & _
                                                    gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("cod_tot_wgt").ToString.Trim, "0")) & ", " & _
                                                    gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("cod_tot_cbm").ToString.Trim, "0")) & ", " & _
                                                    gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_rem").ToString.Trim, ""))) & ", " & _
                                                    gU.convdbNVCData(gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("cod_pallet_no").ToString.Trim, "000"))) & ", " & _
                                                    gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_vnd_code").ToString.Trim, ""))) & ", " & _
                                                    gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("cod_ref_seq").ToString.Trim, "null")) & ", " & _
                                                    gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("ori_batch").ToString.Trim, ""))) & ", " & _
                                                    gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_lot_no").ToString.Trim, ""))) & ", " & _
                                                    gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_carton_no").ToString.Trim, ""))) & ", " & _
                                                    gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_plant").ToString.Trim, ""))) & ", " & _
                                                    gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("cod_pcs_carton").ToString.Trim, "null")) & ", " & _
                                                    gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("cod_carton_pallet").ToString.Trim, "null")) & ", " & _
                                                    "TO_DATE('" & gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("COD_EXPIRY_DATE").ToString.Trim, "")) & "','" & DDFORMAT & "')," & _
                                                    "TO_DATE('" & gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("COD_MANU_DATE").ToString.Trim, "")) & "','" & DDFORMAT & "')," & _
                                                    "'" & Session("usr_id") & "',Getdate(),'" & Session("usr_id") & "',Getdate()) "
                                End Select
                                REM **********************
                            ...
                            If Session("usr_type") = "T" Or Session("usr_type") = "C" Then
                                SendCustCOMail(nextNo, STORER_CODE.Value)
                            End If
                ...
                Else
                    ...
                    REM Modify Here
                    REM char "N" is use for update Unicode
                    sql_string = "update wms_cust_order set " & _
                                    "co_cus_ref_no = " & gU.convdbNVCData(gU.dbEncode(CO_CUS_REF_NO.Text)) & ", " & _
                                    "co_project_no = " & gU.convdbNVCData(gU.dbEncode(CO_PROJECT_NO.Text)) & ", " & _
                                    "co_status = " & gU.convdbNVCData(gU.dbEncode(CO_STATUS.Text)) & ", " & _
                                    "co_date = " & gU.convdbDate(gU.dbEncode(CO_DATE.Text)) & ", " & _
                                    "cus_code = " & gU.convdbNVCData(gU.dbEncode(CUS_CODE.Text)) & ", " & _
                                    "cus_name = " & gU.convdbNVCData(gU.dbEncode(CUS_NAME.Text)) & ", " & _
                                    "co_addr1 = " & gU.convdbNVCData(gU.dbEncode(CO_ADDR1.Text)) & ", " & _
                                    "co_addr2 = " & gU.convdbNVCData(gU.dbEncode(CO_ADDR2.Text)) & ", " & _
                                    "co_addr3 = " & gU.convdbNVCData(gU.dbEncode(CO_ADDR3.Text)) & ", " & _
                                    "co_area_del = " & gU.convdbNVCData(gU.dbEncode(CO_AREA_DEL.Text)) & ", " & _
                                    "co_region_del = " & gU.convdbNVCData(gU.dbEncode(CO_REGION_DEL.Text)) & ", " & _
                                    "co_country_del = " & gU.convdbNVCData(gU.dbEncode(CO_COUNTRY_DEL.Text)) & ", " & _
                                    "co_cus_cont = " & gU.convdbNVCData(gU.dbEncode(CO_CUS_CONT.Text)) & ", " & _
                                    "co_cus_cont_tel = " & gU.convdbNVCData(gU.dbEncode(CO_CUS_CONT_TEL.Text)) & ", " & _
                                    "co_rem = " & gU.convdbNVCData(gU.dbEncode(CO_REM.Text)) & ", " & _
                                    "co_track_no = " & gU.convdbNVCData(gU.dbEncode(CO_TRACK_NO.Text)) & ", " & _
                                    "co_ftrack_no = " & gU.convdbNVCData(gU.dbEncode(CO_FTRACK_NO.Text)) & ", " & _
                                    "co_inv_no = " & gU.convdbNVCData(gU.dbEncode(CO_INV_NO.Text)) & "," & _
                                    "co_cust_inv_no = " & gU.convdbNVCData(gU.dbEncode(CO_CUST_INV_NO.Text)) & "," & _
                                    "co_ship_mode=" & gU.convdbNVCData(gU.dbEncode(CO_SHIP_MODE.SelectedValue)) & "," & _
                                    "CO_TARGET_DELDATE=" & gU.convdbDate(gU.dbEncode(CO_TARGET_DELDATE.Text)) & "," & _
                        ...
                        For i = 0 To tempHOLDDT.Rows.Count - 1
                            ...
                            Select Case tempHOLDDT.Rows(i).Item("mFlag").ToString.Trim
                                Case "N"
                                    ...
                                    maxSeq = DB.getValueFromSQL("Select ISNULL(Max(TO_NUMBER(COH_SEQ)) + 1,1) as value from WMS_CUST_ORDER_HOLD " & _
                                                                " WHERE IMP_CODE='" & Session("IMP_CODE") & "' AND STORER_CODE='" & gU.dbEncode(tempHOLDDT.Rows(i).Item("STORER_CODE").ToString.Trim) & "' " & _
                                    ...
                                    updateHOLDSQL = "insert into wms_cust_order_hold ( wms_cust_order_hold.IMP_CODE,  wms_cust_order_hold.STORER_CODE, " & _
                                                    "            wms_cust_order_hold.CO_CODE,  wms_cust_order_hold.COD_SEQ,  wms_cust_order_hold.COH_SEQ, " & _
                                                    "            wms_cust_order_hold.COH_TYPE,  wms_cust_order_hold.COH_STATUS,  wms_cust_order_hold.RO_CODE, " & _
                                                    "            wms_cust_order_hold.ROD_SEQ,  wms_cust_order_hold.COH_QTY,  wms_cust_order_hold.COH_TOT_PCS, " & _
                                                    "            wms_cust_order_hold.SYS_LUB, " & _
                                                    "            wms_cust_order_hold.SYS_LUD,  wms_cust_order_hold.SYS_CD,  wms_cust_order_hold.SYS_CB, " & _
                                                    "            wms_cust_order_hold.COH_IN_STOCK_QTY) " & _
                                                    " values ( '" & Session("IMP_CODE") & "',  '" & gU.dbEncode(tempHOLDDT.Rows(i).Item("STORER_CODE").ToString.Trim) & "', " & _
                                                    "            '" & gU.dbEncode(tempHOLDDT.Rows(i).Item("CO_CODE").ToString.Trim) & "',  '" & gU.dbEncode(tempHOLDDT.Rows(i).Item("COD_SEQ").ToString.Trim) & "','" & maxSeq & "', " & _
                                                    "            '" & gU.dbEncode(tempHOLDDT.Rows(i).Item("COH_TYPE").ToString.Trim) & "', '" & gU.dbEncode(tempHOLDDT.Rows(i).Item("COH_STATUS").ToString.Trim) & "',  '" & gU.dbEncode(tempHOLDDT.Rows(i).Item("RO_CODE").ToString.Trim) & "', " & _
                                                    "            '" & gU.dbEncode(tempHOLDDT.Rows(i).Item("ROD_SEQ").ToString.Trim) & "', '" & gU.dbEncode(tempHOLDDT.Rows(i).Item("COH_QTY").ToString.Trim) & "',  null, " & _
                                                    "            '" & Session("usr_id") & "', Getdate(),  Getdate(), '" & Session("usr_id") & "', " & _
                                                    "            '" & gU.dbEncode(IN_STOCK_QTY) & "') "
                                ...
                                Case "U"
                                    ...
                                    updateHOLDSQL = "UPDATE wms_cust_order_hold SET " & _
                                                    " COH_STATUS='" & gU.dbEncode(tempHOLDDT.Rows(i).Item("ORI_STATUS").ToString.Trim) & "'," & _
                                                    " RO_CODE='" & gU.dbEncode(tempHOLDDT.Rows(i).Item("RO_CODE").ToString.Trim) & "'," & _
                                                    " ROD_SEQ='" & gU.dbEncode(tempHOLDDT.Rows(i).Item("ROD_SEQ").ToString.Trim) & "'," & _
                                                    " COH_QTY='" & gU.dbEncode(tempHOLDDT.Rows(i).Item("COH_QTY").ToString.Trim) & "'," & _
                                                    tempSQL & _
                                                    " SYS_LUB='" & Session("usr_id") & "'," & _
                                                    " SYS_LUD=Getdate() " & _
                                                    " Where IMP_CODE= '" & Session("IMP_CODE") & "' AND STORER_CODE='" & gU.dbEncode(tempHOLDDT.Rows(i).Item("STORER_CODE").ToString.Trim) & "'" & _
                                                    " AND CO_CODE='" & gU.dbEncode(tempHOLDDT.Rows(i).Item("CO_CODE").ToString.Trim) & "'" & _
                                ...
                                Case "D"
                                    updateHOLDSQL = "Delete From wms_cust_order_hold Where " & _
                                                    " IMP_CODE= '" & Session("IMP_CODE") & "' AND STORER_CODE='" & gU.dbEncode(tempHOLDDT.Rows(i).Item("STORER_CODE").ToString.Trim) & "'" & _
                                                    " AND CO_CODE='" & gU.dbEncode(tempHOLDDT.Rows(i).Item("CO_CODE").ToString.Trim) & "'" & _
                                                    " AND COD_SEQ='" & gU.dbEncode(tempHOLDDT.Rows(i).Item("COD_SEQ").ToString.Trim) & "' AND COH_SEQ='" & gU.dbEncode(tempHOLDDT.Rows(i).Item("COH_SEQ").ToString.Trim) & "' "
                    ...
                    If cU.gfBuildDataTableforGridView(dt, GridView1, True) Then
                        ...
                        For Each rows As DataRow In dt.Rows
                            ...
                            Select Case rows.Item("mFlag")
                                ...
                                Case "D"
                                    itemSQL = "delete from wms_cust_order_d " & _
                                              "where imp_code = '" & gU.dbEncode(imp_code) & "' " & _
                                              "and storer_code = '" & gU.dbEncode(STORER_CODE.Value) & "' " & _
                                              "and co_code = '" & gU.dbEncode(CO_CODE.Text.Trim) & "' " & _
                                              "and cod_seq = '" & gU.dbEncode(gU.decodeNull(rows.Item("cod_seq").ToString.Trim, "")) & "' "

                                    updateHOLDSQL = "Delete from wms_cust_order_hold " & _
                                                    "where imp_code = '" & gU.dbEncode(imp_code) & "' " & _
                                                    "and storer_code = '" & gU.dbEncode(STORER_CODE.Value) & "' " & _
                                                    "and co_code = '" & gU.dbEncode(CO_CODE.Text.Trim) & "' " & _
                                                    "and cod_seq = '" & gU.dbEncode(gU.decodeNull(rows.Item("cod_seq").ToString.Trim, "")) & "' "
                                ...
                                Case Else
                                    itemSQL = "update wms_cust_order_d set " & _
                                                "cod_disp_seq = " & gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_disp_seq").ToString.Trim, ""))) & ", " & _
                                                "cod_job_no = " & gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_job_no").ToString.Trim, ""))) & ", " & _
                                                "cod_ref_no = " & gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_ref_no").ToString.Trim, ""))) & ", " & _
                                                "cod_cust_code = " & gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_cust_code").ToString.Trim, ""))) & ", " & _
                                                "cod_itm_parent = " & gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_itm_parent").ToString.Trim, ""))) & ", " & _
                                                "cod_itm_code = " & gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_itm_code").ToString.Trim, ""))) & ", " & _
                                                "cod_pack_key = " & gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_pack_key").ToString.Trim, ""))) & ", " & _
                                                "cod_itm_desc = " & gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_itm_desc").ToString.Trim, ""))) & ", " & _
                                                "cod_packing = " & gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_packing").ToString.Trim, ""))) & ", " & _
                                                "cod_qty = " & gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("cod_qty").ToString.Trim, "0")) & ", " & _
                                                "cod_uom = " & gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_uom").ToString.Trim, ""))) & ", " & _
                                                "cod_pcs_uom = " & gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("cod_pcs_uom").ToString.Trim, "0")) & ", " & _
                                                "cod_totpcs = " & gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("cod_totpcs").ToString.Trim, "0")) & ", " & _
                                                "cod_tot_wgt = " & gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("cod_tot_wgt").ToString.Trim, "0")) & ", " & _
                                                "cod_tot_cbm = " & gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("cod_tot_cbm").ToString.Trim, "0")) & ", " & _
                                                "cod_rem = " & gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_rem").ToString.Trim, ""))) & ", " & _
                                                "cod_pallet_no = " & gU.convdbNVCData(gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("cod_pallet_no").ToString.Trim, "000"))) & ", " & _
                                                "cod_vnd_code = " & gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_vnd_code").ToString.Trim, ""))) & ", " & _
                                                "cod_ref_seq = " & gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("cod_ref_seq").ToString.Trim, "null")) & ", " & _
                                                "cod_batch_no = " & gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("ori_batch").ToString.Trim, ""))) & ", " & _
                                                "cod_lot_no = " & gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_lot_no").ToString.Trim, ""))) & ", " & _
                                                "cod_carton_no = " & gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_carton_no").ToString.Trim, ""))) & ", " & _
                                                "cod_plant = " & gU.convdbNVCData(gU.dbEncode(gU.decodeNull(rows.Item("cod_plant").ToString.Trim, ""))) & ", " & _
                                                "cod_pcs_carton = " & gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("cod_pcs_carton").ToString.Trim, "null")) & ", " & _
                                                "cod_carton_pallet = " & gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("cod_carton_pallet").ToString.Trim, "null")) & ", " & _
                                                "COD_EXPIRY_DATE= TO_DATE('" & gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("COD_EXPIRY_DATE").ToString.Trim, "")) & "','" & DDFORMAT & "')," & _
                                                "COD_MANU_DATE= TO_DATE('" & gU.dbEncode(gU.decodeNullOrEmpty(rows.Item("COD_MANU_DATE").ToString.Trim, "")) & "','" & DDFORMAT & "')," & _
                                                "sys_lub = '" & Session("usr_id") & "', " & _
                                                "sys_lud = Getdate() " & _
                                                "where imp_code = '" & gU.dbEncode(imp_code) & "' " & _
                                                "and storer_code = '" & gU.dbEncode(STORER_CODE.Value) & "' " & _
                            ...
                            'If Session("usr_type") = "T" Or Session("usr_type") = "C" Then
                            '    SendCustCOMail(CO_CODE.Text.Trim, STORER_CODE.Value)
                            'End If
                ...
                sql_string = "update wms_cust_order_hold " & _
                             "set COH_ITM_CODE = d.COD_ITM_CODE, " & _
                                 "COH_PACK_KEY = d.COD_PACK_KEY, " & _
                                 "COH_PALLET_NO = d.COD_PALLET_NO " & _
                             "from wms_cust_order_d d " & _
                             ...
                             "and wms_cust_order_hold.imp_code = '" & gU.dbEncode(imp_code) & "' " & _
                             "and wms_cust_order_hold.storer_code = '" & gU.dbEncode(STORER_CODE.Value) & "' " & _
    ...
    Protected Sub BindGV()
        ...
        If Session("pagemode") = "N" Then
            ...
            uiFun.load_dropdown(CUS_CODE, "select CUS_CODE, CUS_NAME from wms_customer where " & _
                                          "storer_code = '" & STORER_CODE.Value & "' " & _
                                        "and imp_code = '" & Session("IMP_CODE") & "' " & _
                                        "ORDER BY 2", "CUS_CODE", "CUS_NAME", , Session("gSelectLabel"))

            uiFun.load_dropdown(CO_PROJECT_NO, "select PRJ_CODE, PRJ_NAME from WMS_PROJECT where " & _
                                          "storer_code = '" & STORER_CODE.Value & "' " & _
                                          "and imp_code = '" & Session("IMP_CODE") & "' " & _
        ...
        Else
            ...
            SQLString = " SELECT wms_cust_order.IMP_CODE,  wms_cust_order.STORER_CODE,  wms_cust_order.CO_CODE, " & _
                        "  wms_cust_order.CO_CUS_REF_NO,  wms_cust_order.CO_PROJECT_NO,  wms_cust_order.CO_STATUS, " & _
                        "  TO_CHAR(wms_cust_order.CO_DATE,'" & DDFORMAT & "') as CO_DATE,  wms_cust_order.CUS_CODE,  wms_cust_order.CUS_NAME, " & _
                        "  wms_cust_order.CO_ADDR1,  wms_cust_order.CO_ADDR2,  wms_cust_order.CO_ADDR3, " & _
                        "  wms_cust_order.CO_AREA_DEL,  wms_cust_order.CO_REGION_DEL,  wms_cust_order.CO_COUNTRY_DEL, " & _
                        "  wms_cust_order.CO_CUS_CONT,  wms_cust_order.CO_CUS_CONT_TEL,  wms_cust_order.CO_REM, " & _
                        "  wms_cust_order.CO_TRACK_NO,  wms_cust_order.CO_FTRACK_NO,  wms_cust_order.CO_RO_CODE, " & _
                        "  wms_cust_order.CO_TEAM,  wms_cust_order.CO_SCH_DATE,  wms_cust_order.CO_CONTRACT_TYPE, " & _
                        "  wms_cust_order.CO_AGREEMENT_NO,  wms_cust_order.CO_INSTALLATION_REQ,  wms_cust_order.CO_INITIAL_COLLECT, " & _
                        "  wms_cust_order.CO_INITIAL_COLLECT_AMT,  wms_cust_order.CO_PAY_TERMS,  wms_cust_order.CO_DELIVERED_BY, wms_cust_order.co_ship_mode,to_char(wms_cust_order.CO_TARGET_DELDATE,'" & DDFORMAT & "') as CO_TARGET_DELDATE," & _
                        "  wms_cust_order.SYS_LUB,  wms_cust_order.SYS_LUD,  wms_cust_order.SYS_CD,  wms_cust_order.SYS_CB, " & _
                        "  wms_cust_order.CO_INV_NO,  wms_cust_order.CO_CUST_INV_NO " & _
                        " FROM wms_cust_order " & _
                        "where wms_cust_order.co_code = '" & gU.dbEncode(pk_code) & "' " & _
                        "and wms_cust_order.imp_code = '" & imp_code & "' " & _
            ...
            If dt.Rows.Count > 0 Then
                ...
                PRJ_NAME.Text = DB.getValueFromSQL("SELECT PRJ_NAME FROM WMS_PROJECT WHERE storer_code='" & gU.dbEncode(STORER_CODE.Value) & "' and PRJ_CODE = '" & gU.dbEncode(dt.Rows(0).Item("CO_PROJECT_NO").ToString) & "' ")
                CUS_NAME.Text = DB.getValueFromSQL("SELECT CUS_NAME FROM WMS_CUSTOMER WHERE storer_code='" & gU.dbEncode(STORER_CODE.Value) & "' and CUS_CODE = '" & gU.dbEncode(dt.Rows(0).Item("CUS_CODE").ToString) & "' ")
        ...
        SQLString = " SELECT 'U' as mFlag, wms_cust_order_d.IMP_CODE,  wms_cust_order_d.STORER_CODE,  wms_cust_order_d.CO_CODE,  wms_cust_order_d.COD_SEQ, " & _
                    " wms_cust_order_d.COD_JOB_NO,  wms_cust_order_d.COD_REF_NO,  wms_cust_order_d.COD_DISP_SEQ,  wms_cust_order_d.COD_CUST_CODE, " & _
                    " wms_cust_order_d.COD_CUST_REM,  wms_cust_order_d.COD_ITM_PARENT,  wms_cust_order_d.COD_ITM_CODE,  wms_cust_order_d.COD_PACK_KEY, " & _
                    " wms_cust_order_d.COD_PALLET_NO,  wms_cust_order_d.COD_ITM_DESC,  wms_cust_order_d.cod_post_qty, wms_cust_order_d.COD_PACKING,  wms_cust_order_d.COD_QTY, " & _
                    " wms_cust_order_d.COD_UOM,  wms_cust_order_d.COD_PCS_UOM,  wms_cust_order_d.COD_TOTPCS,  wms_cust_order_d.COD_DELIV_QTY, " & _
                    " wms_cust_order_d.COD_TOT_WGT,  wms_cust_order_d.COD_TOT_CBM,  wms_cust_order_d.COD_REM,  wms_cust_order_d.SYS_LUB, " & _
                    " wms_cust_order_d.SYS_LUD,  wms_cust_order_d.SYS_CD,  wms_cust_order_d.SYS_CB,  wms_cust_order_d.COD_VND_CODE, " & _
                    " wms_cust_order_d.COD_BATCH_NO, wms_cust_order_d.COD_BATCH_NO as ori_batch,  wms_cust_order_d.COD_BATCH_CD,  wms_cust_order_d.COD_LOT_NO,  wms_cust_order_d.COD_CARTON_NO, " & _
                    " wms_cust_order_d.COD_PLANT,  wms_cust_order_d.COD_PCS_CARTON,  wms_cust_order_d.COD_CARTON_PALLET,  wms_cust_order_d.COD_REF_SEQ, " & _
                    " ISNULL(SUM(wms_cust_order_hold.COH_QTY),0) as HOLD_QTY, wms_cust_order_d.COD_ITM_CODE as h_COD_ITM_CODE,  wms_cust_order_d.COD_PACK_KEY as h_COD_PACK_KEY," & _
                    " wms_item.ITM_SKU_NO,wms_item.ITM_DESC,  TO_CHAR(wms_cust_order_d.COD_EXPIRY_DATE,'" & DDFORMAT & "') as COD_EXPIRY_DATE,  TO_CHAR(wms_cust_order_d.COD_MANU_DATE,'" & DDFORMAT & "') as COD_MANU_DATE" & _
                    " FROM wms_cust_order_d, wms_cust_order_hold, wms_item " & _
                    " Where wms_cust_order_d.IMP_CODE  = wms_cust_order_hold.IMP_CODE (+) " & _
                    ...
                    " AND wms_cust_order_d.co_code = '" & gU.dbEncode(pk_code) & "' " & _
                    " and wms_cust_order_d.storer_code = '" & gU.dbEncode(storerCode) & "' " & _
                    " and wms_cust_order_d.imp_code = '" & imp_code & "'" & _
                    " and wms_cust_order_hold.coh_status(+) = 'HOLD'" & _
                    " and wms_cust_order_d.IMP_CODE = wms_item.IMP_CODE (+) " & _
                    " and wms_cust_order_d.STORER_CODE = wms_item.STORER_CODE (+) " & _
                    " and wms_cust_order_d.COD_ITM_CODE = wms_item.ITM_CODE (+) " & _
                    " and wms_cust_order_d.COD_PACK_KEY = wms_item.PACK_KEY (+) " & _
                    " Group by wms_cust_order_d.IMP_CODE,  wms_cust_order_d.STORER_CODE,  wms_cust_order_d.CO_CODE,  wms_cust_order_d.COD_SEQ, " & _
                    "  wms_cust_order_d.COD_JOB_NO,  wms_cust_order_d.COD_REF_NO,  wms_cust_order_d.COD_DISP_SEQ,  wms_cust_order_d.COD_CUST_CODE, " & _
                    "  wms_cust_order_d.COD_CUST_REM,  wms_cust_order_d.COD_ITM_PARENT,  wms_cust_order_d.COD_ITM_CODE,  wms_cust_order_d.COD_PACK_KEY, " & _
                    "  wms_cust_order_d.COD_PALLET_NO,  wms_cust_order_d.COD_ITM_DESC,  wms_cust_order_d.cod_post_qty, wms_cust_order_d.COD_PACKING,  wms_cust_order_d.COD_QTY, " & _
                    "  wms_cust_order_d.COD_UOM,  wms_cust_order_d.COD_PCS_UOM,  wms_cust_order_d.COD_TOTPCS,  wms_cust_order_d.COD_DELIV_QTY, " & _
                    "  wms_cust_order_d.COD_TOT_WGT,  wms_cust_order_d.COD_TOT_CBM,  wms_cust_order_d.COD_REM,  wms_cust_order_d.SYS_LUB, " & _
                    "  wms_cust_order_d.SYS_LUD,  wms_cust_order_d.SYS_CD,  wms_cust_order_d.SYS_CB,  wms_cust_order_d.COD_VND_CODE, " & _
                    "  wms_cust_order_d.COD_BATCH_NO,  wms_cust_order_d.COD_BATCH_CD,  wms_cust_order_d.COD_LOT_NO,  wms_cust_order_d.COD_CARTON_NO, " & _
                    "  wms_cust_order_d.COD_PLANT,  wms_cust_order_d.COD_PCS_CARTON,  wms_cust_order_d.COD_CARTON_PALLET,  wms_cust_order_d.COD_REF_SEQ, wms_item.ITM_SKU_NO, wms_item.ITM_DESC," & _
                    "  TO_CHAR(wms_cust_order_d.COD_EXPIRY_DATE,'" & DDFORMAT & "'), TO_CHAR(wms_cust_order_d.COD_MANU_DATE,'" & DDFORMAT & "') "


        SQLString = SQLString & " order by wms_cust_order_d.cod_disp_seq, TO_NUMBER(wms_cust_order_d.cod_seq) "
        REM **********************
        dt = gDB.getDataTable(SQLString)
        If dt.Rows.Count > 0 Then
            GridView1.DataSource = dt
        ...
        SQLString = " SELECT 'U' as mFlag, 0 as rod_qty, wms_cust_order_hold.IMP_CODE,  wms_cust_order_hold.STORER_CODE,  wms_cust_order_hold.CO_CODE, " & _
                    "  wms_cust_order_hold.COD_SEQ,  wms_cust_order_hold.COH_SEQ,  wms_cust_order_hold.COH_TYPE, " & _
                    "  wms_cust_order_hold.COH_STATUS, wms_cust_order_hold.COH_STATUS as ori_status, wms_cust_order_hold.RO_CODE,  wms_cust_order_hold.ROD_SEQ, wms_cust_order_hold.COH_IN_STOCK_QTY, wms_cust_order_hold.COH_REL_QTY, " & _
                    "  wms_cust_order_hold.COH_QTY,  wms_cust_order_hold.COH_TOT_PCS,  TO_CHAR(wms_cust_order_hold.COH_REL_DATE,'" & DDFORMAT & "') as COH_REL_DATE,  wms_cust_order_hold.COH_LOC, to_char(wms_cust_order_hold.sys_cd,'" & DDFORMAT & "') as sys_cd" & _
                    " FROM wms_cust_order_hold " & _
                    " where wms_cust_order_hold.co_code = '" & gU.dbEncode(pk_code) & "' " & _
                    " and wms_cust_order_hold.storer_code = '" & gU.dbEncode(storerCode) & "' " & _
    ...
    Protected Sub CUS_CODE_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles CUS_CODE.SelectedIndexChanged
        Dim SQLSTR As String = "select cus_name, cus_addr1_del, cus_addr2_del, cus_addr3_del, " & _
                                ...
                                "from wms_customer where cus_code = '" & gU.dbEncode(CUS_CODE.SelectedValue) & "' " & _
                                "and storer_code = '" & STORER_CODE.Value & "' " & _
    ...
    Protected Sub addItemtoCO()
        If cU.gfBuildDataTableforGridView(dt, GridView1, True) Then
            ...
            SQLString = "select WMS_ITEM.ITM_PARENT, WMS_ITEM.ITM_CODE, WMS_ITEM.PACK_KEY, WMS_ITEM.ITM_NAME, WMS_ITEM.ITM_UOM, WMS_ITEM.ITM_PCS_PER_UOM, " & _
                            "WMS_ITEM_LOC_BAL.ILOC_PALLET_NO, WMS_ITEM_LOC_BAL.ILOC_BATCH_NO, WMS_ITEM.ITM_SKU_NO,wms_item.ITM_DESC, v.AITM_QTY_PER_CTN, v.AITM_VOL, v.CARTON_CBM, " & _
                            "WMS_ITEM_LOC_BAL.ITM_CODE || '#_#' || " & _
                                "WMS_ITEM_LOC_BAL.PACK_KEY || '#_#' || " & _
                                "isnull(WMS_ITEM_LOC_BAL.ILOC_PALLET_NO, '') || '#_#' || " & _
                                "isnull(WMS_ITEM_LOC_BAL.ILOC_BATCH_NO, '') as ITM_LOC_KEY, " & _
                                "to_char(WMS_ITEM_LOC_BAL.ILOC_EXPIRY_DATE, '" & gU.dbEncode(DDFORMAT) & "') as ILOC_EXPIRY_DATE, " & _
                                "to_char(WMS_ITEM_LOC_BAL.ILOC_MANU_DATE, '" & gU.dbEncode(DDFORMAT) & "') as ILOC_MANU_DATE " & _
                        "FROM WMS_ITEM " & _
                        "LEFT OUTER JOIN V_ALT_VEND_ITEM v " & _
                        "on WMS_ITEM.IMP_CODE = v.IMP_CODE " & _
                            "and WMS_ITEM.STORER_CODE = v.STORER_CODE " & _
                            "and WMS_ITEM.ITM_CODE = v.ITM_CODE " & _
                            "and WMS_ITEM.PACK_KEY = v.PACK_KEY " & _
                        "LEFT OUTER JOIN ( " & _
                            "select imp_code, " & _
                                "storer_code, " & _
                                "ITM_CODE, " & _
                                "PACK_KEY,ILOC_EXPIRY_DATE, ILOC_MANU_DATE, " & _
                                "ISNULL(ILOC_PALLET_NO, '000') AS ILOC_PALLET_NO, " & _
                                "ISNULL(ILOC_BATCH_NO, '') AS ILOC_BATCH_NO, " & _
                                "ISNULL(VND_CODE, '') AS VND_CODE, " & _
                                "sum(ILOC_BAL_QTY) as TOTAL_BAL_QTY " & _
                            "from WMS_ITEM_LOC_BAL " & _
                            "where ISNULL(ILOC_BAL_QTY, 0) > 0 " & _
                            "and TRIM(WMS_ITEM_LOC_BAL.ITM_CODE || '#_#' || " & _
                                "WMS_ITEM_LOC_BAL.PACK_KEY || '#_#' || " & _
                                "isnull(WMS_ITEM_LOC_BAL.ILOC_PALLET_NO, '') || '#_#' || " & _
                                "isnull(WMS_ITEM_LOC_BAL.ILOC_BATCH_NO, '')) in ('" & Replace(locPackList, ", ", "', '") & "') " & _
                            "group by imp_code, storer_code, ITM_CODE, PACK_KEY,ILOC_EXPIRY_DATE, ILOC_MANU_DATE, ISNULL(ILOC_PALLET_NO, '000'), ISNULL(ILOC_BATCH_NO, ''), ISNULL(VND_CODE, '') " & _
                            ") WMS_ITEM_LOC_BAL " & _
                        ...
                        "where wms_item.storer_code = '" & gU.dbEncode(STORER_CODE.Value) & "' " & _
                        "and wms_item.imp_code = '" & Session("IMP_CODE") & "' " & _
                        "and wms_item.itm_code || '#_#' || wms_item.pack_key in ('" & Replace(itemPackList, ", ", "', '") & "') " & _
            ...
            For i As Integer = 0 To CO_dt.Rows.Count - 1
                ...
                dt.Rows(rows_count - 1).Item("ori_batch") = CO_dt.Rows(i).Item("ILOC_BATCH_NO").ToString.Trim
    ...
    Protected Sub changeLabel()
        ...
        Call cU.newChangeGVLabel(GridView1, "Plan", "Plan")
        Call cU.newChangeGVLabel(GridView1, "Ref. No.", "参考編號")
        'Call cU.newChangeGVLabel(GridView1, "Cust. Code", "客戶號碼")
        'Call cU.newChangeGVLabel(GridView1, "Item Parent", "Item Parent")
        Call cU.newChangeGVLabel(GridView1, "Carton No.", "Carton No.")
        Call cU.newChangeGVLabel(GridView1, "Pallet No.", "板號")
        Call cU.newChangeGVLabel(GridView1, "Batch No.", "Batch No.")
        Call cU.newChangeGVLabel(GridView1, "Lot No.", "Lot No.")
        ...
        Call cU.newChangeGVLabel(GridView1, "Qty/ Carton", "")
        Call cU.newChangeGVLabel(GridView1, "Carton/ Pallet", "")
        ...
        Call cU.newChangeGVLabel(GridView1, "Hold Qty.", "Hold Qty.")
    ...
    Private Sub reOrderCOD(ByRef impDt As DataTable)
        ...
        Dim rowsFound = From c In impDt.Rows _
                          Order By _
                          Convert.ToInt32(c.item("cod_disp_seq")) _
                          Ascending _
                         Select c
    ...
    Protected Sub btnExportTF_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExportTF.Click
        ...
        coitemSQL += "select distinct cod_plant, cod_ref_no, alv_vnd_itmcode, wms_item.itm_code, cod_vnd_code, "
        ...
        coitemSQL += "order by cod_ref_no, cod_plant, wms_item.itm_code, cod_batch_no, cod_lot_no, cod_carton_no, cod_pallet_no "
                    ...
                    Using sw As New StreamWriter(fs, Encoding.UTF8)
                        ...
                        For i As Integer = 0 To coItemTbl.Rows.Count - 1
                            ...
                            If pcsCarton > 0 AndAlso CartonPallet > 0 Then
                                Dim init_reel_id As String = gU.decodeNullOrEmpty(coItemTbl.Rows(i).Item("cod_ref_no").ToString, "")
